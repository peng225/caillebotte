...
const s = (p: p5) => {
  ...
  function init() {
    ...
    markY =
      (distScreenAndFace * virtualPos.y) /
    ...
    for (let i = 0; i < 1000; i++) {
      ...
      const iniY =
        p.random(p.height * (1.0 - markAreaHeightRatio)) -
        (p.height * (1.0 - markAreaHeightRatio)) / 2;
  ...
  function getVirtualPos(screenX: number, screenY: number, eyeX: number, sphereCenter: p5.Vector) {
    const face2ScreenAndFace2VirtualPlaneRatio =
      (distScreenAndFace + distVirtualPlaneAndScreen) / distScreenAndFace;
    ...
    const direction = p.createVector(
      screenX - virtualPlaneX,
      screenY - virtualPlaneY,
      distVirtualPlaneAndScreen
    );
    ...
    const C =
      (virtualPlaneX - sphereCenter.x) ** 2 +
      (virtualPlaneY - sphereCenter.y) ** 2 +
      sphereCenter.z ** 2 -
      r ** 2;
    ...
    return p.createVector(
      virtualPlaneX + direction.x * t,
      virtualPlaneY + direction.y * t,
      direction.z * t
    );
  }
  ...
  function getProjectedXForRightEye(virtualPos: p5.Vector) {
    return (
      (distScreenAndFace * virtualPos.x -
        distBetweenEyes * virtualPos.z +
        distBetweenEyes * distVirtualPlaneAndScreen) /
      (distScreenAndFace + distVirtualPlaneAndScreen - virtualPos.z)
    );
  }
