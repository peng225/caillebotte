  ...
  return (
    <nav className={styles.pagination}>
      {(function () {
        ...
        navItems.push(
          <li key={"prevArrow"}>
            <Link href={pageNames[prevID]}>{"<"}</Link>
          </li>
        );
        for (let i = 0; i < shownIDs.length; i++) {
          ...
          if (i != 0 && id - shownIDs[i - 1] > 1) {
            navItems.push(
              <li key={"dotsBefore" + id.toString()} className={styles.dots}>
                ...
              </li>
            );
          }
          if (id === currentID) {
            navItems.push(
              <li key={"pageID" + id.toString()}>
                <Link href={pageNames[id]} className={styles.focused}>
                  {(id + 1).toString()}
                </Link>
              </li>
            );
            ...
            navItems.push(
              <li key={"pageID" + id.toString()}>
                <Link href={pageNames[id]}>{(id + 1).toString()}</Link>
              </li>
            );
          }
        }
        navItems.push(
          <li key={"nextArrow"}>
            <Link href={pageNames[nextID]}>{">"}</Link>
          </li>
        );
